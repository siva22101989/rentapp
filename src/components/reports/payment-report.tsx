    ...
    const paymentEvents = useMemo(() => {
        ...
        records.forEach(sr => {
            (sr.payments || []).forEach((payment, pIndex) => {
                events.push({
                    ...
                    recordId: sr.id,
                    displayBillNo: String(sr.id).replace(/\D/g, ''),
        ...
        unloadingRecords.forEach(ur => {
            (ur.payments || []).forEach((payment, pIndex) => {
                events.push({
                    ...
                    recordId: ur.id,
                    displayBillNo: String(ur.billNo || ur.id).replace(/\D/g, ''),
                    amount: payment.amount,
                    type: 'unloading',
        ...
        (customerPayments || []).forEach((cp) => {
            events.push({
                ...
                recordId: cp.id, // Store the document ID for deletion
                displayBillNo: cp.refNo || 'BULK',
                amount: cp.amount,
