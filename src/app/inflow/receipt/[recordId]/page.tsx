...
export default function InflowReceiptPage() {
  ...
  const unloadingRecordRef = useMemoFirebase(
    () => (firestore && record?.inflowType === 'Plot' && record.dryingRecordId ? doc(firestore, 'unloadingRecords', record.dryingRecordId) : null),
    [firestore, record]
  );
  const { data: unloadingRecord, loading: loadingUnloadingRecord } = useDoc<UnloadingRecord>(unloadingRecordRef);
  ...
  if (loadingRecord || loadingCustomer || loadingWarehouseInfo || loadingUnloadingRecord) {
