  ...
  ({ records, customer, warehouseInfo, pattiNo, paidNow = 0 }, ref) => {
    ...
    return (
      <div ref={ref} className="bg-white p-6 font-sans text-black w-[190mm] mx-auto print:p-0 print:border-none dialog-print-area">
          ...
          <table className="w-full mb-6 text-[13px] border-collapse border-none">
            <style jsx>{`
              ...
              <tr>
                <td className="py-1 align-top w-[60%]">
                  <table className="w-full no-border">
                    <tbody>
                        <tr><td className="font-bold w-28 uppercase text-[10px] text-slate-500 py-0.5">Bill No</td><td className="py-0.5">: <span className="font-mono font-black text-base">{pattiNo}</span></td></tr>
                        <tr><td className="font-bold w-28 uppercase text-[10px] text-slate-500 py-0.5">Customer</td><td className="py-0.5">: <span className="font-black uppercase">{customer.name}</span></td></tr>
              ...
              <table className="w-full border-2 border-black border-collapse text-[12px]">
                  <thead>
                      <tr className="bg-slate-50 border-b-2 border-black h-10">
                          <th className="font-black uppercase text-[10px] text-center w-[12%]">Lot</th>
                          <th className="font-black uppercase text-[10px] text-center w-[18%]">Inflow</th>
                          <th className="font-black uppercase text-[10px] text-center w-[15%]">Duration</th>
                          <th className="font-black uppercase text-[10px] text-right px-2 w-[12%]">Bags</th>
                          <th className="font-black uppercase text-[10px] text-right px-2 w-[25%]">Rent Calculation</th>
                  ...
                  <tbody>
                      {items.map((item, idx) => (
                          <tr key={idx} className="border-b border-black h-10">
                              ...
                              <td className="text-center font-bold">{item.duration} {item.duration === 1 ? 'Month' : 'Months'}</td>
                      ...
                      <tr className="bg-slate-50 font-black border-t-2 border-black h-10">
                          <td colSpan={3} className="text-right uppercase text-[10px] px-2">Totals</td>
