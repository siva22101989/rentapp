...
const InflowSchema = z.object({
  ...
  commodityDescription: z
    .string()
    .min(2, 'Commodity description is required.'),
  location: z.string(),
  storageStartDate: z
    .string()
  ...
  bagsStored: z.coerce
    .number()
    .int()
    .nonnegative('Number of bags must be a non-negative number.')
    .optional(),
  hamaliRate: z.coerce
    .number()
    .nonnegative('Hamali rate must be a non-negative number.')
    .optional(),
  hamaliPaid: z.coerce
    .number()
    .nonnegative('Hamali paid must be a non-negative number.')
    .optional(),
  ...
  plotBags: z.coerce
    .number()
    .nonnegative('Plot bags must be a non-negative number.')
    .optional(),
  loadBags: z.coerce.number().optional(),
  khataAmount: z.coerce
    .number()
    .nonnegative('Khata amount must be a non-negative number.')
    .optional(),
});
...
export async function addInflow(
  prevState: InflowFormState,
  formData: FormData
) {
  ...
  if (inflowType === 'Plot') {
    if (!plotBags || plotBags <= 0) {
      return {
        message: "Plot Bags must be a positive number for 'Plot' inflow.",
        success: false,
      };
    }
    inflowBags = plotBags;
    ...
    if (!bagsStored || bagsStored <= 0) {
      return {
        message: "Number of Bags must be a positive number for 'Direct' inflow.",
        success: false,
      };
    }
...
const OutflowSchema = z.object({
  ...
  bagsToWithdraw: z.coerce
    .number()
    .int()
    .positive('Bags to withdraw must be a positive number.'),
  withdrawalDate: z
    .string()
    .refine((val) => !isNaN(Date.parse(val)), {
      message: 'Invalid date format',
...
export async function addOutflow(
  prevState: OutflowFormState,
  formData: FormData
) {
  ...
  const { recordId, bagsToWithdraw, withdrawalDate, finalRent, amountPaidNow } =
    validatedFields.data;
  ...
  if (bagsToWithdraw > originalRecord.bagsStored) {
    return {
      message: 'Cannot withdraw more bags than are in storage.',
      success: false,
    };
  }
  ...
  recordUpdate.totalRentBilled =
    (originalRecord.totalRentBilled || 0) + finalRent;
  ...
  redirect(
    `/outflow/receipt/${recordId}?withdrawn=${bagsToWithdraw}&rent=${finalRent}&paidNow=${paymentMade}`
  );
}
...
const StorageRecordUpdateSchema = z.object({
  ...
  hamaliPayable: z.coerce
    .number()
    .nonnegative('Hamali charges must be a non-negative number.'),
  storageStartDate: z
    .string()
    .refine((val) => !isNaN(Date.parse(val)), {
      message: 'Invalid date format',
...
export async function updateStorageRecordAction(
  recordId: string,
  prevState: InflowFormState,
  formData: FormData
) {
...
export async function addPayment(
  ...
  const { recordId, paymentAmount, paymentDate, paymentType } =
    validatedFields.data;
...
export async function deleteStorageRecordAction(
  recordId: string
): Promise<FormState> {
  try {
