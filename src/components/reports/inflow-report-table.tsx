    ...
    return (
        <div className="bg-white p-4 text-black font-sans text-sm printable-area border shadow-sm rounded-lg">
             <div className="mb-6 text-center border-b pb-4">
                <h2 className="text-2xl font-black uppercase tracking-tight leading-none text-center">SRI LAKSHMI WAREHOUSE</h2>
                ...
                <Table className="text-[13px]">
                        ...
                        {records.map((record) => {
                            ...
                            return (
                                <TableRow key={record.id} className="h-9 border-b border-slate-100 hover:bg-slate-50/50 group">
                                    ...
                                    <TableCell className="p-2 font-black whitespace-nowrap uppercase tracking-tighter text-center">{getCustomerName(record.customerId)}</TableCell>
                                    <TableCell className="p-2 font-medium text-slate-600 uppercase text-[12px] text-center">{record.lorryTractorNo || '-'}</TableCell>
                                    <TableCell className="p-2 text-center font-mono font-bold">{record.location || '-'}</TableCell>
                                    <TableCell className="p-2 text-right font-mono font-black text-slate-900 text-center">{record.bagsIn}</TableCell>
                                    <TableCell className="p-2 text-right font-mono text-slate-600 text-center">{record.weight ? `${record.weight}kg` : '-'}</TableCell>
                                    <TableCell className="p-2 text-right font-mono text-slate-400 text-center">{record.hamaliRate ? record.hamaliRate.toFixed(2) : '-'}</TableCell>
                                    <TableCell className="p-2 text-right font-mono font-bold text-center">{formatCurrency(record.hamaliPayable || 0)}</TableCell>
                                    <TableCell className="p-2 text-right font-mono text-center">{record.khataAmount ? formatCurrency(record.khataAmount) : '-'}</TableCell>
                    ...
                    <TableFooter>
                        <TableRow className="bg-slate-50 text-black font-black border-t-2 border-slate-900 h-12 hover:bg-slate-50">
                            ...
                            <TableCell className="p-2 text-right font-mono text-[14px] text-center">{totals.bags}</TableCell>
                            <TableCell className="p-2 text-right font-mono text-[14px] text-center">{totals.weight > 0 ? `${totals.weight}kg` : '-'}</TableCell>
                            <TableCell className="p-2" />
                            <TableCell className="p-2 text-right font-mono text-[14px] text-center">{formatCurrency(totals.hamali)}</TableCell>
                            <TableCell className="p-2 text-right font-mono text-[14px] text-center">{formatCurrency(totals.khata)}</TableCell>
            ...
            <div className="mt-20 flex justify-end">
                <div className="w-72 border-t-2 border-slate-900 text-center pt-2">
                    ...
                    <p className="text-[10px] text-slate-400 mt-1 uppercase text-center">Warehouse Operations Department</p>
                </div>
            </div>
        </div>
    );
